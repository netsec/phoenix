...
try {
    ...
    console.log("WARNING - No png support, maybe need to 'npm update'", e);
}
...
function userCleanup(suser) {
    ...
    if (suser.emailSearch === undefined) {
        suser.emailSearch = false;
    }
    if (suser.removeEnabled === undefined) {
        suser.removeEnabled = false;
    }
...
passport.use(new DigestStrategy({qop: 'auth', realm: Config.get("httpRealm", "Moloch")},
    function (userid, done) {
        Db.getUserCache(userid, function (err, suser) {
            ...
            if (!suser._source.enabled) {
                console.log("User", userid, "not enabled");
                return done("Not enabled");
            }
...
if (Config.get("passwordSecret")) {
    ...
    app.use(function (req, res, next) {
        ...
        if (req.headers['x-moloch-auth']) {
            ...
            Db.getUserCache(obj.user, function (err, suser) {
                if (err) {
                    return res.send("ERROR - x-moloch getUser - user: " + obj.user + " err:" + err);
                }
                if (!suser || !suser.found) {
                    return res.send(obj.user + " doesn't exist");
                }
                if (!suser._source.enabled) {
                    return res.send(obj.user + " not enabled");
                }
                userCleanup(suser._source);
        ...
        if (internals.userNameHeader !== undefined && req.headers[internals.userNameHeader] !== undefined) {
            ...
            Db.getUserCache(userName, function (err, suser) {
                if (err) {
                    return res.send("ERROR - getUser - user: " + userName + " err:" + err);
                }
                if (!suser || !suser.found) {
                    return res.send(userName + " doesn't exist");
                }
                if (!suser._source.enabled) {
                    return res.send(userName + " not enabled");
                }
                if (!suser._source.headerAuthEnabled) {
                    return res.send(userName + " header auth not enabled");
                }
        ...
        passport.authenticate('digest', {session: false})(req, res, function (err) {
            req.url = req.url.replace(Config.basePath(), "/");
            if (err) {
                return res.molochError(200, err);
            }
            else {
                return next();
            }
    ...
    app.use(function (req, res, next) {
        ...
        req.user = {
            userId: username,
            enabled: true,
            createEnabled: username === "anonymous",
            webEnabled: true,
...
function loadFields() {
    Db.loadFields(function (err, data) {
        ...
        app.locals.fieldsArr = Config.getFields().sort(function (a, b) {
    ...
    return function (req, res, next) {
        ...
        function finish() {
            ...
            Db.historyIt(log, function (err, info) {
                if (err) {
                    console.log('log history error', err, info);
                }
            });
        }
...
app.get('/user.css', function (req, res) {
    fs.readFile("./views/user.styl", 'utf8', function (err, str) {
        ...
        if (!req.user.settings.theme) {
            return error('no custom theme defined');
        }

        var theme = req.user.settings.theme.split(':');

        if (!theme[1]) {
            return error('custom theme corrupted');
        }
...
app.post('/user/views/delete', [checkCookieToken, logAction(), postSettingUser], function (req, res) {
    ...
    if (!req.body.view) {
        return res.molochError(403, 'Missing view');
    }

    var user = req.settingUser;
    user.views = user.views || {};
    if (user.views[req.body.view] === undefined) {
        return res.molochError(200, "View not found");
    }
...
app.post('/user/columns/delete', [checkCookieToken, logAction(), postSettingUser], function (req, res) {
    ...
    if (!found) {
        return res.molochError(200, "Column not found");
    }
...
app.post('/user/spiview/fields/create', [checkCookieToken, logAction(), postSettingUser], function (req, res) {
    ...
    if (!req.body.name) {
        return res.molochError(403, 'Missing custom spiview field configuration name');
    }
    if (!req.body.fields) {
...
app.post('/user/spiview/fields/delete', [checkCookieToken, logAction(), postSettingUser], function (req, res) {
    ...
    if (!req.body.name) {
        return res.molochError(403, 'Missing custom spiview fields configuration name');
    }
    ...
    if (!found) {
        return res.molochError(200, "Spiview fields not found");
    }
        ...
        if ((item === "ta" || item === "hh" || item === "hh1" || item === "hh2") && (typeof obj[item] === "string" || Array.isArray(obj[item]))) {
            if (obj[item].indexOf("*") !== -1) {
                ...
                if (item === "ta") {
                    query = {
                        bool: {
                            must: {wildcard: {_uid: "tag#" + obj[item]}},
...
function buildSessionQuery(req, buildCb) {
    ...
    var query = {
        from: req.query.start || req.query.iDisplayStart || 0,
        size: limit,
        query: {bool: {filter: []}}
    };
        ...
        switch (req.query.bounding) {
            ...
            case "database":
                query.query.bool.filter.push({
                    range: {
                        timestamp: {
                            gte: req.query.startTime * 1000,
                            lte: req.query.stopTime * 1000
                        }
                    }
                });
                break;
    ...
    if (req.query.facets) {
        query.aggregations = {
            mapG1: {terms: {field: "g1", size: 1000, min_doc_count: 1}},
        ...
        switch (req.query.bounding) {
            case "first":
                ...
                break;
            case "database":
                query.aggregations.dbHisto.histogram = {
                    field: 'timestamp',
    ...
    try {
        let p = new SyncPromise(function (resolve, reject) {
            try {
                ...
                setTimeout(function(){reject(e)},1);
                console.log(e);
            }
        });
        p.then(function (response) {
            var expr = response;
            var tlpFilter = molochparser.parse(expr);
            query.query.bool.filter.push(tlpFilter);
        ...
        console.log("Couldn't create TLP expression", e);
        err = e;
    }
...
function sessionsListFromIds(req, ids, fields, cb) {
    ...
    var fixFields = nonArrayFields.filter(function (x) {
        return fields.indexOf(x) !== -1;
    });
...
app.delete('/history/list/:id', function (req, res) {
    ...
    if (!req.user.removeEnabled) {
        return res.molochError(403, "Need remove data privileges");
    }
    ...
    Db.indicesCache(function (err, indices) {
        ...
        if (req.query.filter !== undefined) {
            ...
            for (let i = 0, ilen = indices.length; i < ilen; i++) {
                if (!indices[i].index.match(regex)) {
                    continue;
                }
...
app.get('/estask/list', function (req, res) {
    Db.tasks(function (err, tasks) {
        ...
        for (var key in tasks) {
            ...
            if (regex && !task.action.match(regex)) {
                continue;
            }
...
app.get('/esshard/list', function (req, res) {
    Promise.all([Db.shards(),
        ...
        for (var shard of shards) {
            ...
            nodes[shard.node] = {
                ip: shard.ip,
                ipExcluded: ipExcludes.includes(shard.ip),
                nodeExcluded: nodeExcludes.includes(shard.node)
            };
            delete shard.node;
            delete shard.index;
        }

        let indices = Object.keys(result).map((k) => result[k]).sort(function (a, b) {
    ...
    async.parallel({
        ...
        function (err, results) {
            ...
            for (var n = 0, nlen = nodes.length; n < nlen; n++) {
                ...
                if (regex && !node.name.match(regex)) {
                    continue;
                }
...
app.get('/dstats.json', function (req, res) {
    ...
    var mapping = {
        deltaBits: {
            _source: ["deltaBytes"], func: function (item) {
                return Math.floor(item.deltaBytes * 8.0);
            }
        },
        deltaTotalDropped: {
            _source: ["deltaDropped", "deltaOverloadDropped"], func: function (item) {
                return Math.floor(item.deltaDropped + item.deltaOverloadDropped);
            }
        },
        deltaBytesPerSec: {
            _source: ["deltaBytes", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaBytes * 1000.0 / item.deltaMS);
            }
        },
        deltaBitsPerSec: {
            _source: ["deltaBytes", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaBytes * 1000.0 / item.deltaMS * 8);
            }
        },
        deltaPacketsPerSec: {
            _source: ["deltaPackets", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaPackets * 1000.0 / item.deltaMS);
            }
        },
        deltaSessionsPerSec: {
            _source: ["deltaSessions", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaSessions * 1000.0 / item.deltaMS);
            }
        },
        deltaDroppedPerSec: {
            _source: ["deltaDropped", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaDropped * 1000.0 / item.deltaMS);
            }
        },
        deltaFragsDroppedPerSec: {
            _source: ["deltaFragsDropped", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaFragsDropped * 1000.0 / item.deltaMS);
            }
        },
        deltaOverloadDroppedPerSec: {
            _source: ["deltaOverloadDropped", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaOverloadDropped * 1000.0 / item.deltaMS);
            }
        },
        deltaESDroppedPerSec: {
            _source: ["deltaESDropped", "deltaMS"], func: function (item) {
                return Math.floor(item.deltaESDropped * 1000.0 / item.deltaMS);
            }
        },
        deltaTotalDroppedPerSec: {
            _source: ["deltaDropped", "deltaOverloadDropped", "deltaMS"], func: function (item) {
                return Math.floor((item.deltaDropped + item.deltaOverloadDropped) * 1000.0 / item.deltaMS);
            }
        },
        cpu: {
            _source: ["cpu"], func: function (item) {
                return item.cpu * 0.01;
            }
        }
    };
    ...
    var func = mapping[req.query.name] ? mapping[req.query.name].func : function (item) {
...
app.get('/spigraph.json', logAction('spigraph'), function (req, res) {
    ...
    buildSessionQuery(req, function (bsqErr, query, indices) {
        ...
        Db.numberOfDocuments('sessions-*', function (err, total) {
...
app.get('/spiview.json', logAction('spiview'), function (req, res) {
    ...
    buildSessionQuery(req, function (bsqErr, query, indices) {
        ...
        queryValueToArray(req.query.spi).forEach(function (item) {
            var parts = item.split(":");
            if (parts[0] === "fileand") {
                query.aggregations[parts[0]] = {
                    terms: {field: "no", size: 1000},
                    aggs: {fs: {terms: {field: "fs", size: parts.length > 1 ? parseInt(parts[1], 10) : 10}}}
                };
            } else {
...
app.get('/connections.json', logAction('connections'), function (req, res) {
    var health;
    Db.healthCache(function (err, h) {
        health = h;
    });
    buildConnections(req, res, function (err, nodes, links, total) {
        if (err) {
            return res.molochError(403, err.toString());
        }
...
function csvListWriter(req, res, list, fields, pcapWriter, extension) {
    if (list.length > 0 && list[0].fields) {
        list = list.sort(function (a, b) {
            return a.fields.lp - b.fields.lp;
        });
    } else if (list.length > 0 && list[0]._source) {
        list = list.sort(function (a, b) {
            return a._source.lp - b._source.lp;
        });
    }
...
app.get('/uniqueValue.json', logAction(), function (req, res) {
    ...
    if (req.query.type === "tags") {
        query = {
            bool: {
                must: {wildcard: {_uid: "tag#" + req.query.filter + "*"}},
                must_not: {wildcard: {_uid: "tag#http:header:*"}}
            }
        };
    } else {
    ...
    buildSessionQuery(req, function (err, query, indices) {
        ...
        if (req.query.field.match(/^(rawus|rawua)$/)) {
            ...
            if (req.query.field === "ip.src:p1" || req.query.field === "a1:p1") {
                query.aggregations = {
                    field: {
                        terms: {field: "a1", size: aggSize},
                        aggregations: {field2: {terms: {field: "p1", size: 100}}}
                    }
                };
            } else if (req.query.field === "ip.dst:p2" || req.query.field === "a2:p2") {
                query.aggregations = {
                    field: {
                        terms: {field: "a2", size: aggSize},
                        aggregations: {field2: {terms: {field: "p2", size: 100}}}
                    }
                };
            } else {
...
app.post('/user/list', logAction('users'), function (req, res) {
    if (!req.user.createEnabled) {
        return res.molochError(404, 'Need admin privileges');
    }
    ...
    if (req.body.filter) {
        query.query = {
            bool: {
                should: [{wildcard: {userName: "*" + req.body.filter + "*"}},
...
app.post('/user/update', logAction(), checkCookieToken, postSettingUser, function (req, res) {
    ...
    Db.getUser(req.body.userId, function (err, user) {
        ...
        Db.setUser(req.body.userId, user, function (err, info) {
            ...
            return res.send(JSON.stringify({
                success: true,
                text: 'User "' + req.body.userId + '" updated successfully'
            }));
        });
    });
});
...
app.post('/searchAndTag', logAction(), function (req, res) {
    ...
    if (regex.length === 0) {
        return res.molochError(200, "No regex specified");
    }
...
app.get('/:nodeName/scrub/:id', checkProxyRequest, function (req, res) {
    if (!req.user.removeEnabled) {
        return res.molochError(200, "Need remove data privileges");
    }
...
function scrubList(req, res, entire, list) {
    if (!list) {
        return res.molochError(200, "Missing list of sessions");
    }
    ...
    async.eachLimit(list, 10, function (item, nextCb) {
        ...
        return res.end(JSON.stringify({
            success: true,
            text: (entire ? "Deleting of " : "Scrubbing of ") + list.length + " sessions complete"
        }));
    });
}
...
app.post('/receiveSession', function receiveSession(req, res) {
    if (!req.query.saveId) {
        return res.molochError(200, "Missing saveId");
    }
    ...
    function makeFilename(cb) {
        ...
        Db.getSequenceNumber("fn-" + Config.nodeName(), function (err, seq) {
            ...
            saveId.seq = seq;
            Db.indexNow("files", "file", Config.nodeName() + "-" + saveId.seq, {
                num: saveId.seq,
                name: filename,
                first: session.fp,
