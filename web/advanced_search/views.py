...
def get_mongo_data(search, user):
    ...
    cursor = mongo_db.analysis.find(query,
                              {"info":"1", "target.file":"1","behavior.processes": "1", "network.http_ex": "1", "network.https_ex": "1"}).sort("info.ended", -1).limit(200)
    return cursor


def is_greg_or_justin(user):
    return user.username in ["greg", "jb"]
...
def index(request):
    ...
    for mongo_obj in mongo_data:
        ...
        if mongo_obj['info']['category'] == "file":
            new = dict(md5=mongo_obj["target"]["file"]["md5"],
            ...
            if myhttp:
                new["http"] = myhttp
            filename = os.path.basename(mongo_obj["target"]["file"]["name"])
            new.update({"filename": filename})
        analyses_files.append(new)
    moloch_url = 'https://phoenix.beastmode.tools:8005/sessions?date=2180&expression=(tags==[{0}])'.format(",".join(["cuckoo:"+str(analysis) for analysis in analysis_numbers]))
